...
We will see a quick setup to configure the ``borgmatic`` formula for different DNS domain names and several roles.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/borgmatic/parameters/dns:domain/
    ln -s dns:domain /srv/salt/borgmatic/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/borgmatic/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/borgmatic/parameters/dns:domain/example.com.yaml.jinja``:
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/borgmatic/parameters/roles
...
We create a configuration for the role ``borgmatic/server`` in ``/srv/salt/borgmatic/parameters/roles/borgmatic/server.yaml``:
...
We create another configuration for the role ``borgmatic/client`` in ``/srv/salt/borgmatic/parameters/roles/borgmatic/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles borgmatic/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles borgmatic/client
...
.. code-block:: console
    ...
    mv /srv/salt/borgmatic/parameters/dns:domain/  '/srv/salt/borgmatic/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-borgmatic-as-json:
      test.nop:
        - name: {{ borgmatic | json }}
...
Here is an example based on `template-formula/borgmatic/config/file.sls`_:
...
This ``sls`` file expose a ``borgmatic`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ borgmatic['winner'] }}
...
.. _template-formula/borgmatic/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/borgmatic/config/file.sls
